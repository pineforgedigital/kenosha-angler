...
export default function TacticalHUD({ activeModule, setActiveModule }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full z-[120] bg-zinc-950/90 backdrop-blur-xl border-t border-zinc-850 shadow-[0_-12px_40px_rgba(0,0,0,0.7)]">
      <div className="w-full flex justify-start sm:justify-around items-center px-4 py-3 pb-6 md:pb-4 transition-all duration-300 overflow-x-auto whitespace-nowrap scrollbar-hide gap-2 sm:gap-0">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-full font-mono text-[0.6rem] md:text-[0.65rem] font-bold tracking-widest uppercase transition-all duration-300 relative ${
                isActive
